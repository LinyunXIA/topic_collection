...
| [`docs/CLI.md`](docs/CLI.md) | 9 个命令详解：参数、dev/test/prod 示例、退出码、错误码对照 |
...
| `tc-score` | 沙龙话题清单自动打分（六维加权，默认 dry-run，`--apply` 才写） | entry point | [CLI §tc-score](docs/CLI.md#tc-score) |
...
命令清单与 `pyproject.toml [project.scripts]`（`tc-push`/`tc-salon`/`tc-purge`/`tc-extract`/`tc-score`）及 `-m` 模块一致。
