...
```bash
.venv/bin/python -m pytest -q                 # 801 用例全离线，<3s；变慢=打真网了，必须修
...
.venv/bin/tc-salon [--env dev|test|prod] [--dry-run]
...
.venv/bin/tc-score [--env dev|test|prod] [--dry-run|--apply] [--provider minimax|deepseek] [--force]   # 默认 dry-run；--apply 真写话题清单
...
- 模块 ≤200 行（`wc -l feedkicker/*.py`，含 `bitable.py`）；bitable 已拆为 `bitable_{lark,schema,views,records,backfill,reseed}.py` 子模块，另有 `feishu_card_body.py`/`feishu_host.py`/`store_conn.py`/`topic_records.py`（完整清单以 DESIGN §3 为准），`bitable.py` 仅 CLI + facade（DESIGN §21.4），新逻辑加入对应子模块，不堆进 facade。
