...
- **默认 dry-run**：真实写入必须显式 `--apply`；重复运行按 **资讯链接 OR 话题名称** 双键去重跳过（幂等）——话题名经 NFKC 归一、链接经 `canonicalize`（去 fragment、host 小写、保留 query）归一；仅两者皆无命中才写。链接键兜底 LLM 命名非确定性（同一新闻重跑可能得到不同话题名）。
