...
- [ ] F24 `README.md`：定位/环境/安装/配置概览/快速上手/命令总览表/目录导航
- [ ] F25 `docs/CLI.md`：7 命令详解 + dev/test/prod 示例 + 错误码附录（参数/默认/退出码经 `--help`+源码核对）
- [ ] F26 `docs/OPS.md`：配置/凭据/launchd/飞书三坑/排障/环境纪律
- [ ] F27 三件套一致性自检（独立后续任务，本增量不实现）
