...
`trim_url` 只剥成对包裹或后随空白/CJK 的尾标点；query 段内标点不剥，避免 `?q=a!` 与
`?q=a` 去重键碰撞（#R9-19/#R10-20）。
"""
...
def trim_url(token: str) -> str:
    """粘连 token 截到最后一个合法 URL 字符，再剥成对包裹或后随空白/CJK 的尾标点（#R9-19/#R10-20）。
